...
if ( is_admin() ) {
	...
	add_action( "admin_print_styles-$of_page", 'albinomouseoptions_mlu_css', 0 );
	add_action( "admin_print_scripts-$of_page", 'albinomouseoptions_mlu_js', 0 );	
}
...
if ( ! function_exists( 'albinomouseoptions_mlu_init' ) ) :

function albinomouseoptions_mlu_init () {
	register_post_type( 'albinomouseoptions', array(
		'labels' => array(
...
if ( ! function_exists( 'albinomouseoptions_mlu_css' ) ) :

function albinomouseoptions_mlu_css () {
	$_html = '';
...
if ( ! function_exists( 'albinomouseoptions_mlu_js' ) ) :

function albinomouseoptions_mlu_js () {
 ...
 *
 * Dependencies:
 * - albinomouseoptions_mlu_get_silentpost()
 */

if ( ! function_exists( 'albinomouseoptions_medialibrary_uploader' ) ) :

function albinomouseoptions_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
	...
	$int = albinomouseoptions_mlu_get_silentpost( $id );
 ...
 * Return: $_postid.
 *
 * If no "silent" post is present, one will be created with the type "albinomouseoptions"
 * and the post_name of "of-$_token".
 *
 * Example Usage:
 * albinomouseoptions_mlu_get_silentpost ( 'of_logo' );
 */

if ( ! function_exists( 'albinomouseoptions_mlu_get_silentpost' ) ) :

function albinomouseoptions_mlu_get_silentpost ( $_token ) {
	...
	if ( $_token ) {
		...
		$_args = array( 'post_type' => 'albinomouseoptions', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'albinomouseoptions_mlu_insidepopup' ) ) :
function albinomouseoptions_mlu_insidepopup () {
	if ( isset( $_REQUEST['is_albinomouseoptions'] ) && $_REQUEST['is_albinomouseoptions'] == 'yes' ) {
	
		add_action( 'admin_head', 'albinomouseoptions_mlu_js_popup' );
		add_filter( 'media_upload_tabs', 'albinomouseoptions_mlu_modify_tabs' );
	}
}

endif;

if ( ! function_exists( 'albinomouseoptions_mlu_js_popup' ) ) :

function albinomouseoptions_mlu_js_popup () {
	...
	jQuery(function($) {
		...
		// Preserve the "is_albinomouseoptions" parameter on the "delete" confirmation button.
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_albinomouseoptions=yes';
...
if ( ! function_exists( 'albinomouseoptions_mlu_modify_tabs' ) ) :

function albinomouseoptions_mlu_modify_tabs ( $tabs ) {
